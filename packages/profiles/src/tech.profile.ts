...
export const techProfile: IndustryProfilePreset = {
  ...
  workPackages: [
    ...
    },
    {
      ...
      skills: ["status-report", "blocker-digest", "roadmap-summary"],
      subagentConfigs: [],
      permissions: [
        "tool:ci.status:invoke",
        "tool:dashboards.api:invoke",
      ...
      starterPrompts: [
        "Summarize this sprint's status across my active workstreams",
        "What's blocking each of my projects right now?",
        "Draft this week's stakeholder update from our pipeline activity",
      ],
      templateRefs: ["tpl.status-report", "tpl.blocker-digest"],
      minAgentVersion: "1.0.0",
      jobFunctions: ["project_manager"],
    },
    {
