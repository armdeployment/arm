...
/**
 * Map a registry tool to an opencode MCP entry. `http` for MCP/HTTP/connector
 * tools, `stdio` for CLI tools. The Authorization header is an env-var
 * reference only — raw credentials must never appear here.
 */
export function toolToMcpEntry(tool: Tool): Record<string, unknown> {
  const tokenRef = `\${${mcpTokenEnvVar(tool.name)}}`;
  const base = {
    type: tool.kind === "cli" ? "stdio" : "http",
    url: tool.endpoint,
    headers: { Authorization: tokenRef },
  };
  if (tool.kind === "cli") {
    return { ...base, command: tool.endpoint };
  }
  return base;
}
