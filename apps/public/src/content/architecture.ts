...
export const employeePathDiagram: DiagramLabels = {
  ...
  nodes: [
    { id: "questionnaire", label: "Questionnaire", sublabel: "6–9 multiple-choice questions, no free text" },
    ...
    { id: "agent", label: "Configured, metered agent", sublabel: "Package installed, budget attached, stakeholder set" },
  ],
  edges: [
...
export const trustBoundaryDiagram: DiagramLabels = {
  ...
  nodes: [
    { id: "agent", label: "Local agent", sublabel: "opencode / claude code / copilot / Pi, on the employee's machine" },
    {
      id: "proxy",
      label: "Data plane",
      sublabel: "Closed-proxy, open-gateway, resource connectors — inside the tenant VPC. Calls Anthropic/OpenAI or a self-hosted GPU pool from here.",
    },
    { id: "control", label: "Control plane", sublabel: "SaaS by default, or self-hosted single-tenant — policy, billing, audit" },
    { id: "dashboard", label: "Dashboard viewer", sublabel: "A human looking at spend, adoption, and audit" },
  ],
  edges: [
    { from: "agent", to: "proxy", label: "prompts + responses + resource IO" },
    { from: "proxy", to: "control", label: "metadata only — tokens, $, audit decisions (never prompt bodies)" },
...
export const artifactoryDiagram: DiagramLabels = {
  ...
  nodes: [
    ...
    { id: "version", label: "Component version", sublabel: "immutable manifest + optional blob digest" },
    { id: "blob", label: "Content-addressed blob", sublabel: "sha256:<hex> — verified on every read" },
    { id: "package", label: "Work package", sublabel: "pinned versions + routing + budget + permissions" },
...
export const boundaryTable: BoundaryRow[] = [
  { boundary: "Agent → data plane", crosses: "Wire-protocol LLM calls; resource access calls with scoped tokens", neverCrosses: "—" },
  { boundary: "Data plane → providers", crosses: "LLM requests (closed models) or GPU inference (open models); resource IO with minted credentials", neverCrosses: "—" },
  { boundary: "Data plane → control plane", crosses: "Metadata-only events: tokens, cost, audit decisions, agent id, timestamp", neverCrosses: "Prompt bodies, resource content, raw credentials" },
  { boundary: "Control plane → data plane", crosses: "Delegate keys (rotating, short-lived), policy cache", neverCrosses: "Resource content, prompts" },
  { boundary: "Control plane → dashboard viewer", crosses: "Aggregates, per-agent and per-team rollups", neverCrosses: "Prompts, content, secrets" },
];
