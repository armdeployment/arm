...
import { securityHero, invariants, guardrailPhilosophy, deploymentModels, onPremNote } from "@/content/security";
    ...
    <>
      ...
      <Section id="invariants">
        ...
        <ScrollTable>
          <thead>
            <tr>
              <th className="label-meta p-3 text-left" style={{ borderBottom: "1px solid var(--border)", width: "3rem" }}>#</th>
              <th className="label-meta p-3 text-left" style={{ borderBottom: "1px solid var(--border)" }}>Invariant</th>
              <th className="label-meta p-3 text-left" style={{ borderBottom: "1px solid var(--border)" }}>Enforced by</th>
            </tr>
          </thead>
          <tbody>
            {invariants.map((row) => (
              <tr key={row.n}>
                <td className="p-3 text-sm tabular" style={{ borderBottom: "1px solid var(--border)", color: "var(--text-muted)" }}>{row.n}</td>
                <td className="p-3 text-sm" style={{ borderBottom: "1px solid var(--border)" }}>{row.statement}</td>
                <td className="p-3 text-sm" style={{ borderBottom: "1px solid var(--border)", color: "var(--text-secondary)" }}>{row.guardrail}</td>
              </tr>
            ))}
          </tbody>
        </ScrollTable>
      ...
      <Section id="deployment-models">
        ...
        <p className="m-0 mt-6 text-sm" style={{ color: "var(--text-muted)" }}>{onPremNote}</p>
      </Section>
    </>
  );
}
