...
describe("httpSource", () => {
  ...
  it("returns bytes + mediaType on a 200", async () => {
    ...
    const source = httpSource("test", {
      baseUrl: "http://x",
      fetchImpl: fakeFetch({ [digest]: { status: 200, body: "hello" } }),
    });
  ...
  it("throws on a non-404 error status", async () => {
    ...
    const source = httpSource("test", {
      baseUrl: "http://x",
      fetchImpl: fakeFetch({ [digest]: { status: 500 } }),
    });
...
describe("fetchFromSources (tenant backend -> control-plane CDN order)", () => {
  it("returns the first non-null hit", async () => {
    ...
    const cdn: ArtifactSource = {
      name: "cdn",
      fetchBlob: async () => ({ body: new Uint8Array([1]), mediaType: "x" }),
    };
  ...
  it("prefers the tenant backend over the CDN when both have it", async () => {
    let cdnCalled = false;
    const tenant: ArtifactSource = {
      name: "tenant",
      fetchBlob: async () => ({ body: new Uint8Array([9]), mediaType: "tenant" }),
    };
    const cdn: ArtifactSource = {
      name: "cdn",
      fetchBlob: async () => {
