...
describe("S3 connector — mint strategy", () => {
  const req = { agentId: "agt_01", tenantId: "tn_01", bucket: "my-data", actions: ["s3:GetObject"], classificationClearance: "internal" as const };
...
describe("GCS connector — mint strategy", () => {
  const req = { agentId: "agt_01", tenantId: "tn_01", bucket: "data-bucket", actions: ["read"] as ("read")[], classificationClearance: "internal" as const };
...
describe("DB connector — proxy strategy", () => {
  it("proxies a query and returns audit result", async () => {
    const result = await proxyDBQuery({
      agentId: "agt_01", tenantId: "tn_01", dbType: "postgres",
      database: "analytics", query: "SELECT 1", classificationClearance: "internal",
    });
...
describe("SharePoint connector — mint+sync hybrid", () => {
  it("mints a scoped Graph API token", async () => {
    const token = await mintSharePointToken({
      agentId: "agt_01", tenantId: "tn_01", siteUrl: "https://acme.sharepoint.com/sites/eng",
      scopes: ["sites.read", "files.read"], classificationClearance: "internal",
    });
