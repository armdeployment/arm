...
function AdoptionPageContent() {
  ...
  const active = trpc.adoption.activeUsers.useQuery({ scope, jobFunctionKey: jobFunctionKey || null });
    ...
    <div className="space-y-6 p-8">
      <div>
        ...
        <p className="mt-1 text-sm" style={{ color: "var(--text-secondary)" }}>
          Activation funnel, stalls, time-to-value, coverage, and gaps — the product&apos;s report card (A1: adoption at scale is the primary value prop)
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <StatCard label="Activated Seats" value={active.data ? String(active.data.activatedSeats) : "—"} sub="reached first metered call" />
        <StatCard label="Eligible Seats" value={active.data ? String(active.data.eligibleSeats) : "—"} sub="headcount with a published package" />
      </div>

      {/* Hero panel: funnel */}
      <FunnelPanel scope={scope} jobFunctionKey={jobFunctionKey} selectedStep={selectedStep} onStepClick={setSelectedStep} />
