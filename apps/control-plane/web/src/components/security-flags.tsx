          ...
          <div key={flag.id} className="flex items-start gap-3 px-5 py-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <span
                  className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${SEVERITY_STYLES[flag.severity] ?? ""}`}
                >
              ...
              <div className="mt-1.5 flex items-center gap-3">
                ...
                <span
                  className={`rounded px-1.5 py-0.5 text-[9px] font-semibold capitalize ${STATUS_STYLES[flag.status] ?? ""}`}
                >
                  {flag.status}
                </span>
              </div>
            ...
            {/* Severity label */}
            <span
              className={`shrink-0 rounded-full px-2 py-0.5 text-[9px] font-bold capitalize ${SEVERITY_STYLES[flag.severity] ?? ""}`}
            >
              {flag.severity}
            </span>
          </div>
