    ...
    >
      ...
      >
        ...
        <span
          ...
          className="rounded-full  px-2.5 py-0.5 text-[10px] font-semibold text-[var(--accent)]"
        >
          GPU hours
        </span>
      </div>
      ...
      <div className="p-4">
        {data.models.map((m) => (
          <div
            key={m.model}
            className="mb-3 flex items-center gap-3 rounded-md border p-3"
            style={{ borderColor: "var(--border)" }}
          >
            <div
        ...
        <div className="mt-2 flex items-center gap-2 rounded-lg bg-emerald-50 px-3 py-2">
          ...
          <span className="text-xs font-medium text-emerald-700">
            If all traffic self-hosted: ${data.savingsVsApi.toLocaleString()}/mo saved vs API
            pricing
          </span>
        </div>
      </div>
