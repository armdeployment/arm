...
export function LiveTicker() {
  ...
  if (isLoading || !data) {
    return (
      <div
        className="flex items-center gap-3 rounded-md border bg-[var(--bg-elevated)] px-4 py-2 text-xs"
        style={{ borderColor: "var(--border)" }}
      >
        <span
          style={{ backgroundColor: "var(--accent-soft)" }}
          className="h-1.5 w-1.5 animate-pulse rounded-full 0"
        />
  ...
  return (
    ...
    >
      ...
      <Metric
        label="Drift"
        value={`${data.driftPct}%`}
        tone={data.driftPct > 5 ? "danger" : "default"}
      />
