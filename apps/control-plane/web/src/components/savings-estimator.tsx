  ...
  return (
    ...
    >
      ...
      <p className="mt-3 text-xs" style={{ color: "var(--text-secondary)" }}>
        Switch {data.impactedAgents} agents from closed models to open models (GLM-5.2, DeepSeek V3)
        and save
      </p>
      ...
      <div className="mt-4 space-y-1.5">
        <div className="flex items-center justify-between text-[10px]">
          ...
          <span className="font-semibold" style={{ color: "var(--text-secondary)" }}>
            ${data.currentMonthlySpend.toLocaleString()}/mo
          </span>
        </div>
        <div
        ...
        <div className="flex items-center justify-between text-[10px]">
          ...
          <span className="font-semibold text-[var(--success)]">
            ${data.openModelMonthlyEstimate.toLocaleString()}/mo
          </span>
        </div>
        <div
          className="h-1.5 overflow-hidden rounded-full"
        ...
        >
          <div
            ...
            style={{
              width: `${Math.round((data.openModelMonthlyEstimate / data.currentMonthlySpend) * 100)}%`,
            }}
          />
        </div>
      </div>
      ...
      {!switched ? (
        ...
        <div className="mt-4 rounded-md bg-emerald-50 px-4 py-2.5 text-center text-sm font-semibold text-emerald-700">
          ✓ Simulated — policy update logged. Impact preview: $
          {data.potentialSavings.toLocaleString()}/mo saved across {data.impactedAgents} agents.
        </div>
      )}
    </div>
  );
