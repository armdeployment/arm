...
const RECAP = [
  { label: "Questionnaire", detail: "new senior_manager option, live and scored" },
  { label: "Recommendation", detail: "real activation code, real 15-minute expiry" },
  { label: "arm setup", detail: "real redemption — package, components, connections" },
  { label: "arm refine", detail: "pain points + folder + tools, all local (A5)" },
];
  ...
  return (
    <div style={{
        ...
        {RECAP.map((item, i) => {
          ...
          return (
            <div key={item.label} style={{
              ...
              <div style={{ fontSize: 14, fontWeight: 700, color: COLORS.greenDark, fontFamily: FONT_MONO, marginBottom: 8 }}>
                ✓ {item.label}
              </div>
              <div style={{ fontSize: 12, color: COLORS.textDarkMuted, lineHeight: 1.6 }}>{item.detail}</div>
            </div>
          );
        })}
      </div>
      ...
      <div style={{
        ...
        Senior managers — the beachhead — now get a package built for them, end to end
      </div>
    </div>
  );
};
