  ...
  return (
    <div style={{
      ...
      <div style={{ opacity: appear, transform: `translateY(${y}px)`, textAlign: "center" }}>
        ...
        <div style={{ fontSize: 46, fontWeight: 700, color: COLORS.white, letterSpacing: -1.2, lineHeight: 1.12 }}>
          An HR Department for AI Agents
        </div>
        <div style={{ fontSize: 16, color: COLORS.textDarkMuted, marginTop: 20, maxWidth: 820, margin: "20px auto 0", lineHeight: 1.65 }}>
      ...
      <div style={{ display: "flex", gap: 20, marginTop: 40, alignItems: "stretch" }}>
        {PLANES.map((p, i) => (
          <div key={p.name} style={{
            ...
            borderRadius: 12, padding: "20px 26px", minWidth: 330,
          }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: COLORS.white, fontFamily: FONT_MONO }}>{p.name}</div>
            <div style={{ fontSize: 11.5, color: p.color === COLORS.navy ? "#93B4E8" : COLORS.goldLight, marginTop: 5, fontWeight: 600 }}>
              {p.sub}
            </div>
            <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 5 }}>
              {p.items.map((it) => (
                <div key={it} style={{ fontSize: 12, color: COLORS.textDarkMuted, fontFamily: FONT_MONO }}>· {it}</div>
              ))}
            </div>
          </div>
        ))}
      ...
      <div style={{
        ...
        marginTop: 28, fontSize: 12.5, color: COLORS.goldLight, fontFamily: FONT_MONO, textAlign: "center",
      }}>
