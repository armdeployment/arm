    ...
    <div style={{
      ...
      <div style={{ opacity: titleAppear, transform: `translateY(${titleY}px)`, textAlign: "center" }}>
        ...
        <div style={{ fontSize: 16, color: COLORS.textDarkMuted, marginTop: 20, maxWidth: 780, margin: "20px auto 0", lineHeight: 1.6 }}>
          A real conversation with a real LLM, routed through the tenant's own connection —
          and a folder picker that understands you work across more than one project.
          Nothing staged; every screen here is a live capture.
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 16, marginTop: 44 }}>
        {[
          ...
          <div key={item.label} style={{
            opacity: interpolate(frame, [24 + i * 10, 38 + i * 10], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }),
            backgroundColor: COLORS.slate800, border: `1px solid ${COLORS.borderDark}`,
            borderRadius: 10, padding: "18px 24px", textAlign: "center", minWidth: 220,
          }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: COLORS.white, fontFamily: FONT_MONO }}>{item.label}</div>
            <div style={{ fontSize: 12, color: COLORS.goldLight, marginTop: 8, fontWeight: 700 }}>{item.sub}</div>
          </div>
        ))}
      </div>
    </div>
