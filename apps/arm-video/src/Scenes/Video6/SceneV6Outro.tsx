...
export const SceneV6Outro: React.FC = () => {
  ...
  return (
    <div style={{
      ...
      <div style={{ opacity: titleAppear, transform: `translateY(${titleY}px)`, textAlign: "center", marginBottom: 30 }}>
        ...
        <div style={{ fontSize: 32, fontWeight: 700, color: COLORS.white, letterSpacing: -0.5 }}>
      ...
      <div style={{ display: "flex", gap: 20 }}>
        {V6.recap.map((item, i) => (
          <div key={item.unit} style={{
            opacity: interpolate(frame, [16 + i * 8, 30 + i * 8], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }),
            ...
            <div style={{ fontSize: 40, fontWeight: 800, color: COLORS.goldLight, fontFamily: FONT_MONO }}>{item.label}</div>
            <div style={{ fontSize: 11.5, color: COLORS.textDarkMuted, marginTop: 6 }}>{item.unit}</div>
          </div>
        ))}
      </div>

      <div style={{
        opacity: interpolate(frame, [56, 70], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }),
        marginTop: 34, fontSize: 12, color: COLORS.textDarkMuted, fontFamily: FONT_MONO, textAlign: "center",
      }}>
        Same activation code, same session, questionnaire through analysis — real every step
      </div>
    </div>
  );
};
