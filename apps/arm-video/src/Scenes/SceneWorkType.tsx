  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Every prompt classified in microseconds — no tokens spent on
          classification.
          <span style={{ color: COLORS.red, fontWeight: 600 }}>
            {" "}
            Unknown is first-class — never guessed.
          </span>
        </div>
      </div>
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      {REAL.classifierCascade.map((stage, i) => {
        const appear = interpolate(frame, [15 + i * 6, 25 + i * 6], [0, 1], {
          extrapolateLeft: "clamp",
          extrapolateRight: "clamp",
        });
        const x = interpolate(frame, [15 + i * 6, 25 + i * 6], [-30, 0], {
          extrapolateLeft: "clamp",
          extrapolateRight: "clamp",
          easing: Easing.bezier(0.16, 1, 0.3, 1),
        });
        const isQA = stage.stage === "QA";
        const accentColor = isQA
          ? COLORS.textMuted
          : stage.stage === "4"
            ? COLORS.amber
            : COLORS.navy;
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span
                  style={{
                    fontSize: 10,
                    fontFamily: FONT_MONO,
                    color: stage.cost === "$0" ? COLORS.green : COLORS.gold,
                    backgroundColor:
                      stage.cost === "$0"
                        ? "rgba(22,163,74,0.08)"
                        : "rgba(180,83,9,0.08)",
      ...
      >
        ...
        <span style={{ fontSize: 11, color: COLORS.text }}>
          <strong style={{ color: COLORS.red }}>unknown</strong> is stored
          as-is, never coerced — fail-closed per policy at gate time
        </span>
      </div>
    </div>
  );
...
function RealResults({ frame }: { frame: number }) {
  ...
  return (
    <div
      style={{ flex: 1.1, display: "flex", flexDirection: "column", gap: 8 }}
    >
      <div
        style={{
          fontSize: 14,
      ...
      >
        Live Results{" "}
        <span
          style={{ fontSize: 11, fontWeight: 400, color: COLORS.textMuted }}
        >
          (real ClickHouse query)
        </span>
      </div>
      <div style={{ fontSize: 11, color: COLORS.textMuted, marginBottom: 8 }}>
      ...
      <div
        style={{
          display: "grid",
          gridTemplateColumns: "1.4fr 1.2fr 0.7fr 0.5fr",
          gap: 6,
          padding: "0 8px",
          fontSize: 9,
          color: COLORS.textMuted,
      ...
      {/* Event rows */}
      {events.map((evt, i) => {
        const appear = interpolate(frame, [20 + i * 5, 30 + i * 5], [0, 1], {
          extrapolateLeft: "clamp",
          extrapolateRight: "clamp",
        });
          ...
          >
            ...
            <span
              style={{
                ...
                fontFamily: FONT_MONO,
                textAlign: "right",
                color: isUnknown ? COLORS.textMuted : COLORS.green,
                fontWeight: 600,
              }}
            >
      ...
      >
        <div
          style={{ fontSize: 10, color: COLORS.textDarkMuted, marginBottom: 4 }}
        >
          CLICKHOUSE QUERY
        </div>
        <div style={{ fontSize: 11, color: COLORS.textDark, lineHeight: 1.6 }}>
          <span style={{ color: COLORS.goldLight }}>SELECT</span> department,
          work_type, classifier_stage,{"\n"}
          {"        "}work_type_confidence{" "}
          <span style={{ color: COLORS.goldLight }}>FROM</span> arm.llm_events
          {"\n"}
          <span style={{ color: COLORS.goldLight }}>WHERE</span> status ={" "}
          <span style={{ color: COLORS.greenDark }}>'success'</span>
        </div>
      </div>
    </div>
  );
