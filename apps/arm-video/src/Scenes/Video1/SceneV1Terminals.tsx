    ...
    <div style={{
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, width: "100%" }}>
        {V1.employees.map((emp, i) => {
          const appear = interpolate(frame, [8 + i * 7, 20 + i * 7], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
          const y = interpolate(frame, [8 + i * 7, 20 + i * 7], [30, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.bezier(0.16, 1, 0.3, 1) });
            ...
            <div key={i} style={{
              ...
              <div style={{ padding: "10px 12px", fontFamily: FONT_MONO, fontSize: 11.5, lineHeight: 1.75, minHeight: 200 }}>
                {lines.map((line, li) => {
                  ...
                  const isSend = line.includes("⬆️") || line.includes("🔑") || line.includes("🚀");
                  return (
                    <div key={li} style={{
                      opacity: shown, color: isBlock ? COLORS.redDark : isSend ? emp.color : COLORS.textDarkMuted,
      ...
      <div style={{
        ...
        marginTop: 12, fontSize: 10.5, color: COLORS.textMuted,
      }}>
        Real Docker container output · employees on the armtest-internal network · Ollama upstream
      </div>
    </div>
  );
};
