  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 16, width: "100%" }}>
        {V2.profiles.map((p, i) => {
          const appear = interpolate(frame, [8 + i * 8, 22 + i * 8], [0, 1], {
            extrapolateLeft: "clamp",
            extrapolateRight: "clamp",
          });
          const y = interpolate(frame, [8 + i * 8, 22 + i * 8], [30, 0], {
            extrapolateLeft: "clamp",
            extrapolateRight: "clamp",
            easing: Easing.bezier(0.16, 1, 0.3, 1),
          });
          ...
          return (
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                {p.orgNodes.map((node, ni) => {
                  const nodeAppear = interpolate(
                    frame,
                    [18 + i * 8 + ni * 4, 28 + i * 8 + ni * 4],
                    [0, 1],
                    { extrapolateLeft: "clamp", extrapolateRight: "clamp" },
                  );
                  const isContainer =
                    node.name.includes("Plant") ||
                    node.name.includes("HQ") ||
                    node.name.includes("Subsidiary") ||
                    node.name.includes("Corporate");
                  return (
                    <div
                      key={ni}
                      style={{
                    ...
                    >
                      ...
                      <span
                        style={{
                          fontWeight: isContainer ? 700 : 400,
                          color: node.color,
                          flex: 1,
                          whiteSpace: "nowrap",
                          overflow: "hidden",
              ...
              {isHolding && (
                ...
                >
                  Each subsidiary runs its own profile · Chinese walls by
                  default
                </div>
              )}
            </div>
      ...
      >
        Real data from @arm/profiles presets · rendered in the /provisioning
        wizard
      </div>
    </div>
  );
