...
const trafficLights = (
  <div
    style={{ display: "flex", gap: 6, alignItems: "center", paddingLeft: 10 }}
  >
    <div
      style={{
        width: 10,
    ...
    >
      ...
      >
        {[
          ...
          const o = interpolate(frame, [10 + i * 4, 16 + i * 4], [0, 1], {
          ...
          return (
            <div
              ...
              style={{
                opacity: o,
                padding: "3px 12px",
                color: i === 0 ? "#569CD6" : "#999",
                background: i === 0 ? "#37373D" : "transparent",
                borderLeft:
                  i === 0 ? "2px solid #569CD6" : "2px solid transparent",
              }}
            >
              {f}
            </div>
      ...
      >
        {[
          ...
          { text: "", color: "" },
          {
            text: "// REVIEW: PR #3421 — API route validation",
            color: "#6A9955",
          },
          {
            text: "export const createUserSchema = z.object({",
            color: "#DCDCAA",
          },
          { text: "  email: z.string().email(),", color: "#9CDCFE" },
          { text: "  role:  z.enum(['admin', 'user']),", color: "#9CDCFE" },
          {
            text: "  plan:  z.enum(['free', 'pro', 'enterprise']).default('free'),",
            color: "#9CDCFE",
          },
          ...
          {
            text: "  const parsed = createUserSchema.safeParse(body);",
            color: "#9CDCFE",
          },
          { text: "  if (!parsed.success) {", color: "#DCDCAA" },
          {
            text: "    return Response.json({ error: parsed.error }, { status: 400 });",
            color: "#9CDCFE",
          },
          { text: "  }", color: "#DCDCAA" },
          {
            text: "  /* TODO: add rate limiting per ticket PLAT-889 */",
            color: "#6A9955",
            dim: true,
          },
          ...
          const o = interpolate(frame, [15 + i * 3, 20 + i * 3], [0, 1], {
            extrapolateLeft: "clamp",
            extrapolateRight: "clamp",
          });
          return (
      ...
      >
        ...
        <div style={{ fontSize: 10, color: "#F44747", lineHeight: 1.8 }}>
          {[
            { msg: "Type 'undefined' is not assignable...", line: "L23" },
            { msg: "Property 'email' does not exist...", line: "L31" },
          ].map((p, i) => {
            const o = interpolate(frame, [60 + i * 10, 66 + i * 10], [0, 1], {
              extrapolateLeft: "clamp",
              extrapolateRight: "clamp",
            });
            return (
              <div key={i} style={{ opacity: o, display: "flex", gap: 4 }}>
                <span>✕</span>
                <span style={{ flex: 1 }}>{p.msg}</span>
                <span style={{ color: "#888" }}>{p.line}</span>
              </div>
            );
          })}
        </div>
  ...
  <WindowChrome title="OpenCode — arm-tasks" subtitle="Pi">
    ...
    >
      {[
        ...
        {
          text: "  ✓ UserTable.tsx written to src/components/",
          color: "#7EE787",
        },
        ...
        const o = interpolate(frame, [8 + i * 5, 13 + i * 5], [0, 1], {
          extrapolateLeft: "clamp",
          extrapolateRight: "clamp",
        });
        return (
          <div key={i} style={{ opacity: o, color: l.color, height: 25 }}>
            {l.text || "\u00A0"}
          </div>
        );
      })}
  ...
  <WindowChrome title="MES Dashboard — Production Q3" subtitle="OpenCode">
    ...
    >
      <div style={{ display: "flex", gap: 8 }}>
        {[
          { label: "OEE", value: "87%", color: "#22C55E" },
          { label: "THROUGHPUT", value: "1,204", color: "#3B82F6" },
          { label: "SCRAP", value: "2.1%", color: "#EAB308" },
        ].map((k, i) => {
          const o = interpolate(frame, [10 + i * 6, 16 + i * 6], [0, 1], {
            extrapolateLeft: "clamp",
            extrapolateRight: "clamp",
          });
          return (
            ...
            >
              <div
                style={{ fontSize: 9, color: "#94A3B8", fontFamily: FONT_MONO }}
              >
                {k.label}
              </div>
              <div
      ...
      >
        ...
        >
          PRODUCTION LINE — HOURLY
        </div>
        {[
          { name: "Line A", val: 95, color: "#22C55E" },
          { name: "Line B", val: 72, color: "#EAB308" },
          { name: "Line C", val: 88, color: "#3B82F6" },
        ].map((line, i) => {
          const o = interpolate(frame, [25 + i * 6, 31 + i * 6], [0, 1], {
            extrapolateLeft: "clamp",
            extrapolateRight: "clamp",
          });
          const w = interpolate(
        ...
        <div
          style={{
            ...
            opacity: interpolate(frame, [50, 56], [0, 1], {
              extrapolateLeft: "clamp",
              extrapolateRight: "clamp",
            }),
          }}
        >
          <span
            style={{ fontSize: 9, color: "#EF4444", fontFamily: FONT_MONO }}
          >
            ⚠ Budget exceeded: $11.12 / $10.00 (112%) — escalated to approved by
            stakeholder
          </span>
        </div>
      </div>
  ...
  <WindowChrome title="QA Runner — test suite" subtitle="Claude Code">
    ...
    >
      <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
        {[
          { label: "PASSED", value: "14", color: "#22C55E" },
          { label: "FAILED", value: "2", color: "#EF4444" },
          { label: "SKIPPED", value: "1", color: "#EAB308" },
        ].map((s, i) => {
          const o = interpolate(frame, [8 + i * 5, 14 + i * 5], [0, 1], {
            extrapolateLeft: "clamp",
            extrapolateRight: "clamp",
          });
          return (
      ...
      {[
        ...
        const o = interpolate(frame, [15 + i * 4, 20 + i * 4], [0, 1], {
          extrapolateLeft: "clamp",
          extrapolateRight: "clamp",
        });
        return (
...
export const SupplyChainView: React.FC<{ frame: number }> = ({ frame }) => (
  <WindowChrome
    title="Supply Chain Portal — PO-2024-07"
      ...
      >
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            ...
            >
              {["PO #", "Supplier", "Item", "Qty", "Status", "$"].map((h) => (
                <th key={h} style={{ padding: "4px 2px" }}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {[
              ...
              const o = interpolate(frame, [12 + i * 5, 18 + i * 5], [0, 1], {
                extrapolateLeft: "clamp",
                extrapolateRight: "clamp",
              });
              return (
                <tr
                  key={row.po}
                  style={{ opacity: o, borderBottom: "1px solid #F1F5F9" }}
                >
                  <td
                    style={{
                      padding: "3px 2px",
                  ...
                  <td style={{ padding: "3px 2px", color: "#334155" }}>
                    {row.sup}
                  </td>
                  <td style={{ padding: "3px 2px", color: "#334155" }}>
                    {row.item}
  ...
  <WindowChrome title="ARM Remote Access — VPN Client" subtitle="Terminal (Pi)">
    ...
    >
      ...
      >
        ...
        />
        <span style={{ color: "#22C55E", fontWeight: 700, fontSize: 11 }}>
          CONNECTED — arm-vpn.prod (1.2.3.4)
        </span>
      ...
      >
        ...
        {[
          ...
          const o = interpolate(frame, [18 + i * 4, 24 + i * 4], [0, 1], {
            extrapolateLeft: "clamp",
            extrapolateRight: "clamp",
          });
          return (
            <div
              key={k}
              style={{ opacity: o, display: "flex", gap: 8, padding: "2px 0" }}
            >
      ...
      >
        <span style={{ color: "#60A5FA", fontSize: 9 }}>
          → Last task: async data sync via ARM · tokens: 245 · cost: $0.03
        </span>
      </div>
    </div>
  </WindowChrome>
