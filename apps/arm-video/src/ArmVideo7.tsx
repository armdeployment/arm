...
} from "./Scenes/Video7/ShowcaseScene";
...
const SHOWCASE: Array<ShowcaseSceneProps & { name: string; seconds: number }> =
  [
    // ── Act 1 · Employee ──────────────────────────────────────────────────
    {
      ...
      facts: [
        {
          label: "Deterministic by construction",
          detail:
            "score() is pure — no LLM, no I/O. The same answers always rank the same job function, so a manager can be told exactly why someone got a package.",
          tone: "good",
        },
        {
          label: "Structured answers only",
          detail:
            "No free-text field exists in this flow. Invariant 1 holds at the UI layer, not just the API.",
          tone: "accent",
        },
      ],
    },
    {
      ...
      zoom: 1.2,
      caption:
        "Real screenshot — a real 6-character code with a real 15-minute expiry",
      facts: [
        { label: "Recommended", value: "Senior Manager", tone: "neutral" },
        {
          label: "Auto-approves",
          detail:
            "A6: low-risk packages install immediately; higher-risk ones install anyway and gate tool access on approval. The install never blocks.",
          tone: "good",
        },
        {
          label: "One signed generic client",
          detail:
            "Never a per-user compiled binary — the per-user part is the token, not the executable.",
          tone: "accent",
        },
      ],
    },
    {
      ...
      zoom: 1.2,
      caption:
        "Real screenshot — a local server on 127.0.0.1 the CLI just started",
      facts: [
        {
          label: "Zero terminal typing",
          detail:
            "Paste the code or drop the .armsetup file. That is the entire interaction.",
          tone: "good",
        },
        {
          label: "Runtimes come bundled",
          detail:
            "MCPs needing python3 or node get a verified portable build downloaded automatically — never 'go install Python first'.",
          tone: "accent",
        },
      ],
    },
    {
      ...
      zoom: 1,
      caption:
        "Real screenshot — package, budget, 8 components, guided connection steps",
      facts: [
        { label: "Package", value: "senior_manager", tone: "neutral" },
        { label: "Budget", value: "$300/mo", tone: "neutral" },
        {
          label: "No secret ever written to disk",
          detail:
            "Configs carry env-var references only; the token lands in .arm-env at mode 0600. assertNoSecretsInConfig fails the install otherwise.",
          tone: "accent",
        },
      ],
    },
    {
      ...
      facts: [
        {
          label: "Chat runs through the tenant's own proxy",
          detail:
            "Same armProxyUrl + agentToken as every other tool call. Never a third party, never ARM's control plane.",
          tone: "accent",
        },
        {
          label: "Folders read extensions only",
          detail:
            "Never file names, never contents. All local. Only the derived tags travel.",
          tone: "good",
        },
      ],
    },
    ...
    // ── Act 2 · Manager ───────────────────────────────────────────────────
    {
      ...
      facts: [
        ...
        {
          label: "Top stall cause",
          detail:
            "MDM push failed on corporate device (35) — the funnel names WHERE and WHY people fall out, not just that spend is low.",
          tone: "accent",
        },
      ],
    ...
    },
    {
      ...
      facts: [
        {
          label: "Material Planner over cap",
          value: "$612 / $600",
          tone: "neutral",
        },
        {
          label: "Cost per work product",
          detail:
            "$214 per 8D report — with a rework-rate counterweight, so re-opened work re-burns tokens and the number stays honest.",
          tone: "accent",
        },
        {
          label: "One-tap approvals",
          detail:
            "The package is the unit of governance: budget, approval, and metering all attach to it.",
          tone: "good",
        },
      ],
    },
    {
      ...
      facts: [
        {
          label: "22 nodes · 3 plants",
          detail:
            "Manufacturing HQ + plants, holding-company subsidiaries, or a flat fintech with Chinese walls — provisioned from industry profiles.",
          tone: "neutral",
        },
        {
          label: "Profiles set defaults, never gate",
          detail:
            "Runtime code never reads the profile id. A tech tenant can define a plant_manager role too — enforced by the no-profile-branching guardrail.",
          tone: "accent",
        },
      ],
    },
    ...
    // ── Act 3 · Server / library ──────────────────────────────────────────
    {
      ...
      facts: [
        {
          label: "A package is a role's whole toolkit",
          detail:
            "MCP tools, skills, sub-agents, permissions, model routing, budget template, starter prompts — pinned to exact versions.",
          tone: "neutral",
        },
        {
          label: "Content-addressed",
          detail:
            "Every version carries a sha256 over its canonical manifest. The client recomputes it and refuses to install on mismatch.",
          tone: "accent",
        },
      ],
    },
    {
      ...
      imagePosition: "50% 0%",
      caption:
        "Real screenshot — facet counts computed over real Postgres rows",
      facts: [
        {
          label: "Kinds",
          detail:
            "21 CLI · 20 skill · 15 http_api · 13 template · 5 subagent · 2 MCP · 2 connector · 1 prompt pack",
          tone: "neutral",
        },
        {
          label: "Source",
          value: "78 / 1",
          detail:
            "first_party vs tenant_authored — provenance is a first-class facet, not a footnote.",
          tone: "accent",
        },
      ],
    },
    {
      ...
      facts: [
        {
          label: "Promote writes a real component",
          detail:
            "review_status='draft', source_kind='imported' — it enters the same signed pipeline as anything first-party.",
          tone: "good",
        },
        {
          label: "Built for the tools with no MCP yet",
          detail:
            "Plenty of engineering software has none. Candidates can be drafted from vendor docs, then reviewed — never auto-trusted.",
          tone: "accent",
        },
      ],
    },
    {
      ...
      facts: [
        {
          label: "requested → approved → active → revoked",
          detail:
            "Every transition is a real row with an approver and a timestamp — this is the audit trail, not a log line.",
          tone: "accent",
        },
        {
          label: "Assignees are users, org nodes, or agents",
          detail: "A whole plant can hold a package, not just a person.",
          tone: "neutral",
        },
      ],
    },
...
export const ARM_VIDEO_7_DURATION =
  (INTRO_SECONDS + CARD_SECONDS * 3 + OUTRO_SECONDS) * FPS +
  showcaseFrames -
  (SHOWCASE.length + 4) * T;
...
export const ArmVideo7: React.FC = () => {
  ...
  SHOWCASE.forEach((s, i) => {
    ...
    beats.push({
      key: `showcase-${i}-${s.name}`,
      seconds: s.seconds,
      node: <ShowcaseScene {...s} />,
    });
  });
    ...
    <TransitionSeries>
      {beats.flatMap((b, i) => [
        ...
        <TransitionSeries.Sequence
          key={b.key}
          durationInFrames={b.seconds * FPS}
          name={b.key}
        >
          {b.node}
        </TransitionSeries.Sequence>,
